...
class LicenciaControlador():
    ...
    def addPersona(self, newEmpleado):
        ...
        if (empleadoRepetido == None):
            ...
            print(
                f"\t\t└>Se insertó el empleado '{newEmpleado.getNombreApe()}' con nro de legajo '{newEmpleado.getNroLegajo()}'\n")
        else:
            print(f"\t\t└>Ocurrio un error al intentar crear el empleado"
                  f" (Ya existe un empleado con el mísmo nro de legajo '{newEmpleado.getNroLegajo()}')\n")
    ...
    def delPersona(self, nroLegajoBuscado):
        ...
        if (personaBorrar != None):
            ...
            print(
                f"\t\t└>Se colocó el estado de '{personaBorrar.getEstado()}' al empleado '{personaBorrar.getNombreApe()}'"
                f" con nro de legajo '{personaBorrar.getNroLegajo()}'")
        else:
            print(f"\t\t└>Ocurrio un error al intentar dar de baja el empleado"
                  f" (No se encontró el empleado con el nro de legajo '{nroLegajoBuscado}')")
    ...
    def generarDias_correspondiente(self, nroLegajoBuscado, diasCorrespNew):
        ...
        if (empleado != None):
            ...
            if (diasDuplicado == None):  # si no esta repetido
                ...
                print(f"\t\t└>Se insertó el día correspondiente del año '{diasCorrespNew.getFecha().strftime('%Y')}'"
                      f" con '{diasCorrespNew.getDias()}'"
                      f" días disponibles al legajo nro '{nroLegajoBuscado}'\n")
            else:
                print(f"\t\t└>Ocurrio un error al intentar dar días al empleado"
                      f" (Ya existen días asociados al año '{diasCorrespNew.getFecha().strftime('%Y')}' al nro de legajo)")
        else:
            print(f"\t\t└>Ocurrio un error al intentar dar días al empleado"
                  f" (No se encontró el empleado con el nro de legajo '{nroLegajoBuscado}')")
    ...
    def bajaDias_correspondiente(self, nroLegajoBuscado, diasCorrespBaja):
        ...
        if (empleado != None):
            if (empleado.buscarDias_correspondiente(diasCorrespBaja) != None):
                ...
                print(
                    f"\t\t└>Se colocó el estado de '{empleado.buscarDias_correspondiente(diasCorrespBaja).getEstado()}'"
                    f"al día correspondiente del año "
                    f"'{empleado.buscarDias_correspondiente(diasCorrespBaja).getFecha().strftime('%Y')}'")
            else:
                print(f"\t\t└>Ocurrio un error al intentar dar de baja los días al empleado"
                      f" (No se encontraron los días del año '{diasCorrespBaja.getFecha().strftime('%Y')}'")
        else:
            print(f"\t\t└>Ocurrio un error al intentar dar de baja los días al empleado"
                  f" (No se encontró el empleado con el nro de legajo '{nroLegajoBuscado}')")
    ...
    def generarLicencia(self, nroLegajoBuscado, newLicencia):
        ...
        if (empleado != None):
            ...
            if (licdupli is None):  # si la Lic no existe
                if (self.verificarCantidadDiasLic(empleado.getDiasCorrespondienteList(),
                                                  newLicencia) == True):  # si hay suficientes dias para la lic
                    ...
                    print(f"\t\t└>Se insertó la licencia del '{newLicencia.getFechaIni().strftime('%d/%m/%Y')}' "
                          f"al empleado '{empleado.getNombreApe()}'\n")
            else:
    ...
    def insertarDiasALicencia(self, nroLegajoBuscado, newLicencia):
        ...
        for dias in empleado.getDiasCorrespondienteList():
            while ((cantDiasPedidos > 0) and (
                    dias.getEstado() == True)):  # si no se tomaron la cantidad de días para la lic, y ese dia corresp
            ...
            for dTomadosEnLic in newLicencia.getDiasTomados():
                ...
                print(f"\t\t\t└>La licencia '{newLicencia.getFechaIni().strftime('%d/%m/%Y')}' tiene"
                      f" '{dTomadosEnLic.getCantidadDiasTomados()}' días "
    ...
    def actualizarVencimientosDiasCorresp(self, empleadoAct, CantAnioVencimientoSem):
        ...
        for dias in empleadoAct.getDiasCorrespondienteList():
            ...
            if (noUsados == True):  # si no se ocupo ese día en ninguna licencia
                ...
                #si no se usó y los dias ya superan 5 años de antiguedad desde la fecha actual
                if (dias.getFecha()+datetime.timedelta(weeks=CantAnioVencimientoSem) < datetime.datetime.now()):
                    dias.setEstado(False)
                    print(f"\t\t└>Los días correspondiente '{dias.getFecha().strftime('%Y')}'"
                          f" pasó la fecha de vencimiento de '{CantAnioVencimientoSem}' semanas admitidas sin licencia asociadas\n\t\t"
